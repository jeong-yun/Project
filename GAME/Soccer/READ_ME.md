...
- 이 문서 하나로 프로젝트 전체(목표·설계·구현·실행·한계·다음 단계)를 파악할 수 있도록 작성했습니다.
...
헤드리스 검증(디스플레이 없이 로직만): `python soccer.py --test 6000`

---

## 8. 코드 맵 (`soccer.py` 탐색 가이드)
...
스탯을 물리·확률로 변환하고, 사람/AI를 Intent 레이어로 통일해 **한 엔진으로 두 모드**를 돌리는 탑다운 축구 프로토타입. 핵심 규칙(오프사이드·파울/카드·세트피스)까지 동작하며, 상태가 2.5D로 설계되어 추후 3D 그래픽 교체가 열려 있다.
